...
const CaregiverBottomNavigation = ({ activeTab, onTabChange, onBack, onSignOut }: CaregiverBottomNavigationProps) => {
  const tabs = [
    ...
    <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-brand-grey/30 safe-area-pb z-50">
      <div className="w-full max-w-md mx-auto px-2 sm:px-4 py-2">
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={onBack}
            className="flex flex-col items-center py-2 px-2 rounded-lg transition-all duration-200 text-brand-charcoal/60 hover:text-brand-charcoal"
          >
          ...
          <button
            onClick={onSignOut}
            className="flex flex-col items-center py-2 px-2 rounded-lg transition-all duration-200 text-brand-charcoal/60 hover:text-brand-red"
          >
